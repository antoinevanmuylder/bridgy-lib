...
sort : FKCtx → Level
sort [] = ℓ-zero
sort (l ∷ ls) = ℓ-max (sort ls) (ℓ-suc (toLvl l))

-- [1, 6, 2] ↦ the ROTT context ( α:Type₂ , β  : Type₆ , γ : Type₁ ) ⊨ ...
⟦_⟧kctx : ∀ (Θ : FKCtx) → NRGraph (sort Θ)
⟦_⟧kctx [] = topNRG
...
------------------------------------------------------------------------
-- SysF types and their translation.

-- what about type levels??
data FType (Θ : FKCtx) : Type  where
  Ftyvar : Fin (length Θ) → FType Θ   --de bruijn indices
  _⟶_ : FType Θ → FType Θ → FType Θ
