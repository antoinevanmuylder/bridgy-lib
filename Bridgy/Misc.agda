...
--   ListRCover : List A → List A → Type ℓ
--   ListRCover = ListRCover' A A (BridgeP (λ _ → A))

--   -- The bridge at Type corresponding to the above relation λ as0 as1 → as0 [ListRCover A] as1
--   ListRCoverBdg : BridgeP (λ _ → Type ℓ) (List A) (List A)
--   ListRCoverBdg = λ x → primGel (List A) (List A) (ListRCover) x

--   -- -- nil "constructor" for primGel _ _ ListRCover x
--   -- nilx : (@tick x : BI) → primGel _ _ ListRCover x
--   -- nilx x = prim^gel [] [] _ x

--   -- cons "constructor" for primGel _ _ ListRCover x
--   consx : (@tick x : BI) →
--           (primGel A A (BridgeP (λ _ → A)) x) → -- "hd"
--           (primGel (List A) (List A) (ListRCover) x) → -- "tl"
--           (primGel (List A) (List A) (ListRCover) x)
--   consx x ghd gtl =  -- ghd, gtl implicitly depend on x
--     {!!}

--     -- primExtent
--     -- {A = λ x → primGel A A (BridgeP (λ _ → A)) x}
--     -- {B = λ x a → primGel (List A) (List A) (ListRCover) x} {!!} {!!}
--     --     -- goal is now reduced to (hd:A) (gtl: Gel_x ListRCover) → Gel_x ListRCover
--     --     (λ hd0 hd1 ghdd y →
--     --     {!primExtent
--     --     {A = λ y → primGel _ _ ListRCover y}
--     --     {B = λ y t → primGel _ _ ListRCover y} ? ?
--     --     ?
--     --     x
--     --     gtl!})
--     -- x
...
--   loosen : ∀ as0 as1 → ListRCover as0 as1 → BridgeP (λ _ → List A) as0 as1
--   loosen [] [] = λ _ → (λ _ → [])
--   loosen [] (_ ∷ _) = λ ctr → rec (ctr .lower)
--   loosen (_ ∷ _) [] = λ ctr → rec (ctr .lower)
--   loosen (hd0 ∷ tl0) (hd1 ∷ tl1) = λ hd-tll → λ x → (hd-tll .fst x) ∷  loosen _ _ (hd-tll .snd) x


--   -- -- we obtain a function Bridge_ListA ? ? → ListRCover ? ?
-- --   -- by ungelling tighten0
-- --   tighten0 :  List A → (@tick x : BI) → primGel (List A) (List A) ListRCover x
-- --   tighten0 [] x = prim^gel [] [] _ x
-- --   -- the encoding of the (hd ∷ _) function on (primGel _ _ (ListRCover) x)    (for hd, x fixed).
-- --   -- ie we seek a "hd-consing" function (primGel _ _ (ListRCover) x) → (primGel _ _ (ListRCover) x)
-- --   -- It is obtained by first building a bdg btw hd‌∷_ and hd∷ _ (with extent) and second x to this bdg.
-- --   tighten0 (hd ∷ tl) x =
-- --     primExtent {A = λ x → primGel (List A) (List A) ListRCover x} {B = λ x a → primGel (List A) (List A) ListRCover x}
-- --      (λ atl → hd ∷ atl)
-- --      (λ atl → hd ∷ atl)
-- --      (λ ts0 ts1 tss → λ y → prim^gel {R = ListRCover} (hd ∷ ts0) (hd ∷ ts1) ( (λ _ → hd) , prim^ungel {R = ListRCover} (λ z → tss z) ) y )
-- --      x
-- --      (tighten0 tl x)

-- --   ListvsBridge0 : ∀ as0 as1 → ListRCover as0 as1 ≃ BridgeP (λ _ → List A) as0 as1
-- --   ListvsBridge0 as0 as1 = isoToEquiv (iso
-- --     (loosen as0 as1)
-- --     (λ q → {!prim^ungel {R = ListRCover} (λ x → tighten0 (q x) x)!}) {!!} {!!})

-- -- --  y → prim^gel {R = ListRCover} (hd ∷ tl) (hd ∷ tl) ( (λ _ → hd) , {!prim^ungel {R = ListRCover} (λ z → tss z)!} ) y
-- -- -- prim^ungel {R = ListRCover} (λ z → tss z)
-- -- -- primExtent {A = λ _ → List A} {B = λ x _ → primGel (List A) (List A) ListRCover x}
-- -- --                            (λ _ → hd ∷ tl) (λ _ → hd ∷ tl)

-- --   -- nil nil case.
-- --   loosenNilNil : ListRCover [] [] → BridgeP (λ _ → List A) [] []
-- --   loosenNilNil _ = λ _ → []
...
-- --   -- consx : (@tick x : BI) (hd : A) (tl : List A) → ListRCoverBdg x
-- --   -- consx x hd tl = {!prim^gel!}

-- --   -- listTighten : (@tick x : BI) → List A → primGel (List A) (List A) (ListRCover) x
-- --   -- listTighten x [] = prim^gel [] [] _ x
-- --   -- listTighten x (hd ∷ tl) = {!primExtent {A = λ _ → A} {B = λ x _ → primGel (List A) (List A) ListRCover x}
-- --   --                             !}
